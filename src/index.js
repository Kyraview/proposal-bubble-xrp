...
export class Wallet{
    ...
    async enable(opts){
      ...
      if(!genisisIdProvided || !genisisHashProvided || !accountsProvided){
        ...
        this.injector.inject(importWalletButton,"width: 50px; height: 50px; cursor:pointer; margin: 5px; filter: invert(100%);");
    ...
    handlePostMessage(message){
      ...
      if(message.hasOwnProperty('width')){
        this.bubble.setWidth(message.width)
      }
      if(message.hasOwnProperty('height')){
        this.bubble.setHeight(message.height)
      }
